...
static struct ch34x_buf *
ch34x_buf_alloc( unsigned int size )
{
    struct ch34x_buf *pb;
...
static void
ch34x_buf_free( struct ch34x_buf *pb )
{
    if( pb ) {
        kfree( pb->buf_buf );
        kfree( pb );
...
static void
ch34x_buf_clear( struct ch34x_buf *pb )
{
    if( pb != NULL )
...
static unsigned int
ch34x_buf_data_avail( struct ch34x_buf *pb )
{
    if( pb == NULL )
        return 0;
...
static unsigned int
ch34x_buf_space_avail( struct ch34x_buf *pb )
{
    if( pb == NULL )
        return 0;
...
static int
ch34x_vendor_read( __u8 request,
        __u16 value,
        __u16 index,
...
static int
ch34x_get_baud_rate( unsigned int baud_rate,
        ...
        struct ktermios        *old_termios )
{
    ...
    dbg_ch34x("%s - data bits = %d",
            __func__, reg_value + 0x05 );
...
static int
ch34x_attach( struct usb_serial *serial )
{
...
static void
ch34x_write_bulk_callback( struct urb *urb )
{
    struct usb_serial_port *port   = (struct usb_serial_port *)urb->context;
    struct ch34x_private   *priv   = usb_get_serial_port_data(port);
    int                     retval;
    int                      tatus = urb->status;
    ...
    switch( status ) {
        ...
        case -ESHUTDOWN:
            ...
            dbg_ch34x("%s - urb shutting down with status:%d",
                    __func__, status);
            priv->write_urb_in_use = 0;
            return;
        default:
            ...
            dbg_ch34x("%s - nonzero write bulk status received:%d",
                    __func__, status);
